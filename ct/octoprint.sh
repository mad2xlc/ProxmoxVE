...
source <(curl -fsSL https://raw.githubusercontent.com/mad2xlc/ProxmoxVE/refs/heads/octoprint/misc/build.func)
...
echo -e "${TAB}${GATEWAY}${BGN}http://${IP}:5000${CL} and ${BGN}http://${IP}${CL} if HAproxy was installed."
echo -e "${INFO}${YW} Access it using one of the following URLs:${CL}"
echo -e "${TAB}${GATEWAY}${BGN}http://${IP}:5000${CL}"
echo -e "${TAB}or with HAproxy"
